...
def main():
    ...
    create_folder(os.path.join(args.save_dir, "runs/hr"))
    create_folder(os.path.join(args.save_dir, "runs/sr"))
...
def train_psnr(dataloader: torch.utils.data.DataLoader,
    ...
    for i, (lr, hr) in enumerate(dataloader):
        ...
        if iters % 300 == 0:
            vutils.save_image(hr.detach(), os.path.join(args.save_dir, "runs", "hr", f"PSNR_{iters}.bmp"))
            vutils.save_image(sr.detach(), os.path.join(args.save_dir, "runs", "sr", f"PSNR_{iters}.bmp"))
...
def train_gan(dataloader: torch.utils.data.DataLoader,
    ...
    for i, (lr, hr) in enumerate(dataloader):
        ...
        # Save image every 300 batches.
        if iters % 300 == 0:
            vutils.save_image(hr.detach(), os.path.join(args.save_dir, "runs", "hr", f"GAN_{iters}.bmp"))
            vutils.save_image(sr.detach(), os.path.join(args.save_dir, "runs", "sr", f"GAN_{iters}.bmp"))
